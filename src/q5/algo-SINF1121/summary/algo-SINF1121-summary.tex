...
private int hash(Key x) {
	return (x.hashCode() && 0x07fffffff) % M);
}
\end{lstlisting}
...
\underlined{Note:} Kruskal's algorithm is generally slower than Prim's algorithm because it has to do a \texttt{connected()} operation for each edge, in addition to the priority-queue operations that both algorithms do for each edge processed.
